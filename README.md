...
```
argocd-apps/
├── applications/                  # ArgoCD Application 定义清单
...
按以下顺序依次 apply ArgoCD Application 清单：

```bash
# 1. 部署监控栈（Prometheus + Grafana）
kubectl apply -f applications/prometheus-app.yaml

# 2. 部署日志栈
kubectl apply -f applications/loki-app.yaml
kubectl apply -f applications/promtail-app.yaml

# 3. 部署链路追踪
kubectl apply -f applications/tempo-app.yaml

# 4. 部署 OTel 数据网关
kubectl apply -f applications/otel-collector-app.yaml

# 5. 部署渐进式发布控制器
kubectl apply -f applications/argorollouts-app.yaml
